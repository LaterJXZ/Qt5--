    ...
    ui(new Ui::TcpClient)
{
    ...
    connect(tcpClient,&QTcpSocket::connected,this,&TcpClient::test);
}
...
{
    on_tcpClientCloseButton_clicked();
}
...
void TcpClient::readMessage()
{
    ...
    double speed = bytesReceived / usedTime;
    ...
    if(bytesReceived == totalBytes){
        localFile->close();
        tcpClient->close();
        ui->tcpClientStatusLabel->setText(tr("接受文件%1完毕").arg(fileName));
    }
}
...
void TcpClient::on_tcpClientCancleButton_clicked()
{
    tcpClient->abort();
    if(localFile->isOpen())
        localFile->close();
...
void TcpClient::on_tcpClientCloseButton_clicked()
{
    tcpClient->abort();
    if(localFile->isOpen())
        localFile->close();
